...
public class Board {
	...
	// Tromp-Taylor rules (captures not included)
		...
		for(int i = 0; i < boardSize; ++i) {
			for(int j = 0; j < boardSize; ++j) {
				if(getColor(i, j) == color) {
					...
					while(!s.empty()) {
						Point cur = s.pop();
						char curColor = getColor(cur.x, cur.y);
						if(curColor == Game.OUT_OF_BOUNDS) {
							continue;
						}
						if(curColor == Game.EMPTY) {
							if(!visited[cur.x][cur.y]) {
								empty++;
								Point[] surrounding = getSurrounding(cur.x, cur.y);
								for(int k = 0; k < surrounding.length; ++k) {
									s.push(surrounding[k]);
								}
								visited[cur.x][cur.y] = true;
							}
						} else if(curColor == Game.BLACK) {
							black++;
						} else if(curColor == Game.WHITE) {
							white++;
						}
					}
					if(color == Game.BLACK && white == 0 && black > 0) {
						points += empty;
					} else if(color == Game.WHITE && black == 0 && white > 0) {
						points += empty;
					}
				}
			}
